...
- `difficulty`は evaluating-difficulty Skill に従い、公開ソースの CEFR から判定する。
...
`difficulty`は [evaluating-difficulty Skill](.agents/skills/evaluating-difficulty/SKILL.md) に従い、ネット上の公開ソースから機械的に判定する。

| 対象 | 参照ソース |
| --- | --- |
| word | Cambridge Dictionary の CEFR → Oxford 3000 / 5000 で照合 |
| phrase | Oxford Phrase List → なければ Cambridge Dictionary |

CEFR から Difficulty への変換:
...
1. Cambridge Dictionary で CEFR を取得する（word）。
2. Oxford 3000 / 5000 で CEFR を取得する（word）。
3. 両方ある場合は一致を確認する。片方しかない場合はその値を採用する。
4. phrase は Oxford Phrase List を第一候補とする。
5. どちらにも CEFR がない場合のみ推測する。

複数の CEFR が付いている場合、または語義ごとにレベルが異なる場合は、理解に必要な知識が最も多い Difficulty を選ぶ。
